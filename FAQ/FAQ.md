...
2. Generate a dedicated **App Password** from Google Account Security → 2-Step Verification → App passwords.
...
**A:** Yes, significantly. Sending emails from your own domain such as `updates@yourcompany.com` via your verified SMTP provider (Amazon SES, SendGrid, Postmark) builds domain authority and reduces the likelihood of notifications landing in your customers' spam or promotions folders.
...
**A:** User segments allow you to gate specific boards to matching logged-in end-users (for example, Beta Testers, Enterprise Tier, EU Customers). If a user does not match the active segment criteria attached to a board, the board will be hidden from their view entirely.
...
### Q: What is the difference between Port 587 and Port 465, and which one should choose?
...
### Q: Can I use my custom SMTP for specific email types only (for example, only announcements, but not transactional password resets)?
...
**A:** Yes. Connecting your custom SMTP server ensures your emails come from your domain, while configuring your CNAME records routes portal traffic to your custom Web URL (for example, roadmap.yourdomain.com). Combining both creates a completely white-labeled customer portal experience.
...
*_A:_ No. ProductBridge relies on four fixed system roles: Owner, Admin, Editor, and Viewer. Custom role definitions and per-board or per-project permission scoping are currently unsupported.
...
*_A:_ Yes. JWT generation occurs strictly on your application server using your private Widget API Secret. The secret is never sent to or stored in client-side code.
...
### Q: If you enable Hide user identities, can workspace administrators still identify who submitted a specific piece of feedback?
